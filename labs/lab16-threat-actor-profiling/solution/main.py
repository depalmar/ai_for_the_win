...
class TTPExtractor:
    ...
    TECHNIQUE_PATTERNS = {
        ...
        'T1059': (r'command.?line|powershell|cmd\.exe|bash|script|wscript|cscript', 'Command and Scripting Interpreter'),
        ...
        'T1078': (r'valid account|credential|stolen password|compromised credential', 'Valid Accounts'),
        'T1021': (r'remote service|rdp|ssh|winrm|smb|remote desktop', 'Remote Services'),
        'T1071': (r'application layer protocol|http|https|dns tunnel|c2 over', 'Application Layer Protocol'),
        'T1105': (r'ingress tool transfer|download|stage|retrieve payload', 'Ingress Tool Transfer'),
        'T1027': (r'obfuscat|encod|pack|encrypt|xor|base64', 'Obfuscated Files or Information'),
        'T1082': (r'system information discovery|systeminfo|hostname|environment', 'System Information Discovery'),
        'T1083': (r'file.{0,10}directory discovery|dir |ls |find |enumerate file', 'File and Directory Discovery'),
        'T1057': (r'process discovery|tasklist|ps aux|process list', 'Process Discovery'),
        'T1003': (r'credential dump|mimikatz|lsass|sam|ntds|password hash', 'OS Credential Dumping'),
        'T1486': (r'data encrypted|ransomware|encrypt files|ransom', 'Data Encrypted for Impact'),
        'T1490': (r'inhibit system recovery|delete shadow|vssadmin|backup delete', 'Inhibit System Recovery'),
        'T1547': (r'boot.*autostart|registry run|startup folder|persistence', 'Boot or Logon Autostart Execution'),
...
class ThreatActorClusterer:
    ...
    def cluster_campaigns(self, campaigns: List[Campaign],
                          threshold: float = 0.4) -> List[List[Campaign]]:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                infra_sim = self.calculate_infrastructure_overlap(campaigns[i].iocs, campaigns[j].iocs)

                # Target overlap
                targets1 = set(campaigns[i].targets)
                targets2 = set(campaigns[j].targets)
                target_sim = len(targets1 & targets2) / max(len(targets1 | targets2), 1) if targets1 or targets2 else 0
...
class MalwareAttributor:
    ...
    def find_similar_samples(self, sample: MalwareSample,
                             threshold: float = 0.3) -> List[Tuple[MalwareSample, float]]:
    ...
    def analyze_incident(self, incident_data: dict) -> dict:
        ...
        if attributions:
            ...
            if top.confidence > 0.7:
                results['confidence_assessment'] = f"High confidence attribution to {top.actor_name}"
            elif top.confidence > 0.4:
                results['confidence_assessment'] = f"Medium confidence attribution to {top.actor_name}"
            else:
                results['confidence_assessment'] = "Low confidence - insufficient matching indicators"
        else:
        ...
        'actors': [
            ...
            },
            {
                ...
                'ttps': [
                    {'technique_id': 'T1566', 'technique_name': 'Phishing', 'tactic': 'Initial Access'},
                    {'technique_id': 'T1059', 'technique_name': 'Command and Scripting Interpreter', 'tactic': 'Execution'},
                    {'technique_id': 'T1053', 'technique_name': 'Scheduled Task', 'tactic': 'Persistence'},
