        ...
        sigma_templates = {
            ...
            "T1003.001": {
                ...
                "detection": {
                    ...
                    "filter": {
                        "SourceImage|endswith": ["\\wmiprvse.exe", "\\svchost.exe"]
                    },
...
class PurpleTeamReporter:
    ...
    def _generate_executive_summary(
        self, exercise_name: str, detection_rate: float,
        critical_gaps: List[str], score: float
    ) -> str:
...
""".strip()
    ...
    def _generate_recommendations(
        self, findings: List[PurpleTeamFinding]
    ) -> List[str]:
        """Generate prioritized recommendations."""
        recommendations = []
        ...
        if partial:
            recommendations.append(
                f"Tune {len(partial)} partially-effective detections"
            )
