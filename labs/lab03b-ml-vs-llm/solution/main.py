...
def extract_ml_features(log_text: str) -> list:
    ...
    has_encoded = 1 if ("encoded" in log_lower or "-enc " in log_lower or "base64" in log_lower) else 0
...
def create_llm_prompt(log_text: str) -> str:
    ...
    return f"""You are an expert security analyst. Classify this log entry as either MALICIOUS or BENIGN.
...
def compare_approaches(logs: list) -> dict:
    ...
    test_logs = [logs[i] for i in range(len(logs)) if i >= len(logs) * 0.7]
...
def hybrid_classifier(log_text: str, model, threshold_low: float = 0.3, 
                      threshold_high: float = 0.8) -> tuple:
    """
...
def main():
    ...
    feature_names = ["has_failed", "has_privileged", "keyword_count", 
                     "log_length", "has_external_ip", "has_encoded"]
    print("\n  Feature Importance:")
    ...
    print(f"  {'Accuracy':<20} {ml_acc:.1%}{'':<10} {llm_acc:.1%}{'':<10} {'LLM' if llm_acc > ml_acc else 'ML':<10}")
    
    ml_time = comparison['ml']['time'] * 1000
    llm_time = comparison['llm']['time']
    print(f"  {'Speed (100 logs)':<20} {ml_time:.1f}ms{'':<8} {llm_time:.0f}s{'':<10} {'ML':<10}")
