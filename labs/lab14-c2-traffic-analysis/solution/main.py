...
class DNSTunnelDetector:
    ...
    def analyze_query(self, query: str) -> dict:
        ...
        if subdomain and all(c in 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=' for c in subdomain.replace('.', '')):
            indicators.append("Base64-like encoding detected")

        # Check for hex-like patterns
        if subdomain and all(c in '0123456789abcdefABCDEF.' for c in subdomain):
    ...
    def detect_tunneling_domain(self, queries: List[dict],
        ...
        for domain, stats in domain_stats.items():
            ...
            if confidence > 0.3:
                candidates.append(TunnelingCandidate(
                    domain=domain,
...
class HTTPC2Detector:
    ...
    C2_URI_PATTERNS = [
        '/submit.php', '/pixel.gif', '/__utm.gif',
        '/login.php', '/admin.php', '/upload.php',
        '/jquery-', '.js?', '/api/v1/', '/beacon'
    ]
...
class C2DetectionPipeline:
    ...
    def analyze_traffic(self, traffic_data: dict) -> C2Report:
        ...
        # 1. Beacon detection
        beacons = self.beacon_detector.analyze_all_pairs(
            traffic_data.get('connections', [])
        )

        # 2. DNS tunneling
        tunneling = self.dns_detector.detect_tunneling_domain(
            traffic_data.get('dns', [])
        )

        # 3. HTTP C2 patterns
        http_c2 = []
        for session in traffic_data.get('http_sessions', []):
            result = self.http_detector.analyze_http_session(
                session.get('flows', [])
            )
...
def main():
    ...
    if report.beacons:
        ...
        for b in report.beacons:
            ...
            print(f"    Interval: {b.interval:.1f}s, Jitter: {b.jitter:.1%}, Confidence: {b.confidence:.1%}")

    if report.tunneling:
        print(f"\n--- DNS Tunneling ({len(report.tunneling)}) ---")
        for t in report.tunneling:
            print(f"  {t.domain}")
            print(f"    Queries: {t.query_count}, Entropy: {t.avg_entropy:.2f}, Confidence: {t.confidence:.1%}")
