...
CRITICAL_EVENTS = {
    ...
    "PutBucketPolicy", "PutBucketAcl",  # Storage exposure
}

HIGH_RISK_EVENTS = {
    "CreateUser", "CreateLoginProfile",  # New identities
    "CreateAccessKey",  # New credentials
    "AttachUserPolicy", "AttachRolePolicy",  # Permission grants
...
CROSS_CLOUD_EVENTS = {
    ...
    "network_change": {
        "aws": ["AuthorizeSecurityGroupIngress", "AuthorizeSecurityGroupEgress", "CreateSecurityGroup"],
...
def normalize_cloud_event(event: Dict, cloud_provider: str) -> Dict:
    ...
    if cloud_provider == "aws":
        ...
        return {
            ...
            "resource": event.get("requestParameters", {}).get("bucketName") or 
                       event.get("requestParameters", {}).get("userName") or "N/A",
            "source_ip": event.get("sourceIPAddress"),
