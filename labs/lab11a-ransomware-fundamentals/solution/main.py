...
def identify_ransomware_family(artifacts: RansomwareArtifacts) -> Dict:
    ...
    for family, sigs in FAMILY_SIGNATURES.items():
        if artifacts.encrypted_extension.lower() in [ext.lower() for ext in sigs["extensions"]]:
            ...
            matched_indicators.append(f"Extension match: {artifacts.encrypted_extension} -> {family.value}")
    ...
    for family, sigs in FAMILY_SIGNATURES.items():
        for note_file in sigs["note_files"]:
            if artifacts.ransom_note_filename.lower() == note_file.lower():
                ...
                matched_indicators.append(f"Note filename match: {artifacts.ransom_note_filename} -> {family.value}")
...
MITRE_TECHNIQUES = {
    ...
    "adfind": {"id": "T1087.002", "name": "Domain Account Discovery (AdFind)", "tactic": "Discovery"},
    "network_scan": {"id": "T1046", "name": "Network Service Discovery", "tactic": "Discovery"},
    "file_discovery": {"id": "T1083", "name": "File and Directory Discovery", "tactic": "Discovery"},
    ...
    "rclone": {"id": "T1567.002", "name": "Exfiltration to Cloud Storage", "tactic": "Exfiltration"},
    "exfil_cloud": {"id": "T1567", "name": "Exfiltration Over Web Service", "tactic": "Exfiltration"},
    ...
    "cobalt_strike": {"id": "T1071.001", "name": "Application Layer Protocol", "tactic": "Command and Control"},
}
...
def recommend_recovery_approach(scenario: IncidentScenario) -> Dict:
    ...
    if "HIPAA" in scenario.regulatory_requirements:
        result["regulatory_actions"].append(
            "HIPAA: Report breach to HHS within 60 days"
        )
        result["regulatory_actions"].append(
