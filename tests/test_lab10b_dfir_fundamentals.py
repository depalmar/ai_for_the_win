...
# Add labs to path
sys.path.insert(0, str(Path(__file__).parent.parent / "labs" / "lab10b-dfir-fundamentals" / "solution"))
...
def test_process_analysis_detects_office_shell():
    ...
    processes = [
        {"pid": 100, "name": "powershell.exe", "parent": "WINWORD.EXE",
         "path": "C:\\Windows\\System32\\powershell.exe", "cmdline": "powershell.exe"}
    ]
    
    findings = analyze_processes(processes)
    assert len(findings) > 0
    assert any("office" in f.description.lower() or "spawn" in f.description.lower() 
               for f in findings)
...
def test_process_analysis_detects_masquerading():
    ...
    processes = [
        {"pid": 100, "name": "svchost.exe", "parent": "cmd.exe",
         "path": "C:\\Users\\Public\\svchost.exe", "cmdline": "svchost.exe"}
    ]
...
def test_file_analysis_detects_temp_exe():
    ...
    files = [
        {"path": "C:\\Windows\\Temp\\malware.exe", "size": 100000, "created": "2024-01-15"}
    ]
...
def test_network_analysis_detects_suspicious_port():
    ...
    assert len(findings) > 0
    assert any("4444" in f.description or "suspicious" in f.description.lower() 
               for f in findings)
